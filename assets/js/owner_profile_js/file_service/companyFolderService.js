...
class CompanyFolderService {
    ...
    /**
     * ============ QOVLUQLARI GƏTİR (loadCompanyFolders) ============
     * @param {string} companyCode - Şirkət kodu
     */
    async loadCompanyFolders(companyCode) {
        console.log(`📂 loadCompanyFolders çağırıldı: ${companyCode}`);
        ...
        try {
            ...
            // API sorğusu - düzgün endpoint
            const url = `${this.baseUrl}/api/v1/company-folders?company_code=${companyCode}`;
            console.log('📡 URL:', url);
     ...
     * @param {string} search - Axtarış sözü
     */
    async getFolders(companyCode, parentId = null, search = null) {
        ...
        try {
            ...
            // Hər qovluğa əlavə məlumatlar əlavə et
