...
class TaskManager {
    ...
    async loadActiveTasks(page = 1, forceRefresh = false) {
        try {
            console.log(`📋 Aktiv tasklar yüklənir... (forceRefresh: ${forceRefresh})`);
            ...
            if (!forceRefresh) {
                ...
                if (allTasks) {
                    console.log('✅ Cache-dən aktiv tasklar yükləndi');

                    // 🔥 CRITICAL: Cache-dən gələn məlumatın array olduğuna əmin ol
                    if (!Array.isArray(allTasks)) {
                        console.warn('⚠️ Cache-dəki məlumat array deyil, object-dir. Boş array ilə əvəz edilir');
                        console.log('📦 Cache məzmunu:', allTasks);

                        // Əgər object-dirsə, içindən data array-ni tapmağa çalış
                        if (allTasks.data && Array.isArray(allTasks.data)) {
                            allTasks = allTasks.data;
                        } else if (allTasks.items && Array.isArray(allTasks.items)) {
                            allTasks = allTasks.items;
                        } else if (allTasks.tasks && Array.isArray(allTasks.tasks)) {
                    ...
                    const endpoint = `/tasks/detailed?page=${page}&limit=${this.pagination.active.pageSize}&status=pending,in_progress,waiting,overdue,pending_approval,paused`;
                    const response = await this.apiRequest(endpoint, 'GET');
                    allTasks = response?.data || response || [];

                    // API-dən gələn cavabın array olduğuna əmin ol
                    if (!Array.isArray(allTasks)) {
                        if (allTasks.data && Array.isArray(allTasks.data)) {
                            allTasks = allTasks.data;
                        } else if (allTasks.items && Array.isArray(allTasks.items)) {
                    ...
                    console.log(`✅ API-dən ${allTasks.length} task yükləndi və cache edildi`);
                }
            } else {
                // Force refresh
                console.log('🔄 Force refresh: API-dən yenilənir...');
                const endpoint = `/tasks/detailed?page=${page}&limit=${this.pagination.active.pageSize}&status=pending,in_progress,waiting,overdue,pending_approval,paused`;
                const response = await this.apiRequest(endpoint, 'GET');
                allTasks = response?.data || response || [];
                ...
                TaskCache.set('active_tasks', allTasks);
                console.log(`✅ Force refresh: ${allTasks.length} task yeniləndi`);
            }

            // Cədvəli yenilə
            const activeTasks = allTasks.filter(t =>
                ['pending_approval', 'pending', 'in_progress', 'overdue', 'waiting', 'paused'].includes(t.status)
            );
    ...
    async checkAndArchiveOverdueTasks() {
        try {
            ...
            // ✅ Cache-dən yoxla, boşdursa API-dən götür
            let allTasks = TaskCache.get('active_tasks');

            if (!allTasks) {
                console.log('🔄 Cache boşdur, overdue yoxlama üçün API-dən yüklənir...');
                const endpoint = `/tasks/detailed?limit=1000&status=pending,in_progress,waiting,overdue,pending_approval,paused`;
                const response = await this.apiRequest(endpoint, 'GET');
                allTasks = response?.data || response || [];
                TaskCache.set('active_tasks', allTasks);
            }
            ...
            if (overdueCount > 0) {
                console.log(`✅ ${overdueCount} task overdue edildi:`, overdueTaskIds);

                // 🔥 CACHE-İ TƏMİZLƏ - çünki məlumatlar dəyişdi
                TaskCache.clear();

                // Yeni məlumatları yüklə
    ...
    setupAutoArchiveCheck() {
        setTimeout(() => this.checkAndArchiveOverdueTasks(), 10000);
