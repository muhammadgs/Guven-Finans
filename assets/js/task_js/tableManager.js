...
const TableManager = {
    ...
    renderTasksTable: function (tableType, tasks, append = false, currentPage = 1) {
        ...
        // Hər task üçün sətir yarat
        const rowPromises = tasks.map(async (task, index) => {
            try {
                return await this.createTaskRow(tableType, task, index, currentPage);
            } catch (error) {
        ...
        // Bütün sətirləri gözlə və əlavə et
        Promise.all(rowPromises).then(rows => {
            ...
            // ƏGƏR ACTIVE TABLE-DİRSƏ, TASK SAYINI YENİLƏ
            if (tableType === 'active') {
    ...
    // ==================== TASK ROW CREATION ====================
    createTaskRow: async function (tableType, task, index, currentPage) {
        const row = document.createElement('tr');

        try {
            // 🔥 YALNIZ CACHE-DƏ YOXDURSA API-DƏN YÜKLƏ
            if (tableType === 'active' && (!task.attachments || task.attachments === '[]' || (Array.isArray(task.attachments) && task.attachments.length === 0))) {
                try {
                    console.log(`🔄 Task ${task.id} üçün attachments yüklənir...`);
                    ...
                    if (response && !response.error) {
                        ...
                        if (taskDetail.attachments) {
                            ...
                            console.log(`✅ Task ${task.id} attachments yükləndi:`, task.attachments);
                        }
                    }
                } catch (e) {
                    console.error(`❌ Task ${task.id} attachments yüklənərkən xəta:`, e);
                }
            }

            
            // Attachments artıq task-da varsa istifadə et, yoxdursa boş array
            if (!task.attachments) {
                task.attachments = [];
            }
            ...
            switch (tableType) {
                case 'active':
                    if (window.ActiveRowCreator && typeof window.ActiveRowCreator.createActiveRowHTML === 'function') {
                        html = window.ActiveRowCreator.createActiveRowHTML(task, index, currentPage);
                    } else {
                        console.error('❌ ActiveRowCreator.createActiveRowHTML funksiyası mövcud deyil!');
                        html = this.createFallbackActiveRowHTML(task, index, currentPage);
                    }
                    row.innerHTML = html;
                    break;
                ...
                case 'archive':
                    if (window.ArchiveTasks && typeof window.ArchiveTasks.createArchiveRowHTML === 'function') {
                        html = window.ArchiveTasks.createArchiveRowHTML.call(this, task, index, currentPage);
                    } else {
                        console.warn('⚠️ ArchiveTasks tapılmadı, fallback istifadə olunur');
                        html = this.createFallbackArchiveRowHTML(task, index, currentPage);
                    }
                    row.innerHTML = html;
                    break;

                case 'external':
                    if (window.ExternalTableManager && typeof window.ExternalTableManager.createFallbackRow === 'function') {
                        const externalRow = window.ExternalTableManager.createFallbackRow(task, index);
                        if (typeof externalRow === 'string') {
                            row.innerHTML = externalRow;
                        } else {
                            console.warn('⚠️ ExternalTableManager.createFallbackRow string qaytarmadı');
                            html = this.createFallbackExternalRowHTML(task, index, currentPage);
                            row.innerHTML = html;
                        }
                    } else {
                        console.warn('⚠️ ExternalTableManager tapılmadı, fallback istifadə olunur');
                        html = this.createFallbackExternalRowHTML(task, index, currentPage);
                        row.innerHTML = html;
                    }
                    break;
                ...
                case 'partner':
                    if (window.PartnerRowCreator && typeof window.PartnerRowCreator.createPartnerRowHTML === 'function') {
                        const result = window.PartnerRowCreator.createPartnerRowHTML(task, index, currentPage);
                        ...
                        console.error('❌ PartnerRowCreator.createPartnerRowHTML funksiyası mövcud deyil!');
                        html = this.createFallbackPartnerRowHTML(task, index, currentPage);
                    }
                    row.innerHTML = html;
                    break;
            ...
            // Task ID-ni data atributu kimi əlavə et
            if (!row.hasAttribute('data-task-id') && task.id) {
                row.setAttribute('data-task-id', task.id);
            }

        } catch (error) {
            console.error(`❌ ${tableType} sətir yaradılarkən xəta:`, error);
            row.innerHTML = `<td colspan="20" style="color: #ef4444; text-align: center;">Xəta: ${error.message}</td>`;
        }
    ...
    showTaskFiles: function(taskId) {
        if (window.FileUploadManager && typeof window.FileUploadManager.showTaskFiles === 'function') {
            window.FileUploadManager.showTaskFiles(taskId);
        } else {
            console.error('❌ FileUploadManager.showTaskFiles tapılmadı');
            this.showError('Fayl göstərmə modulu yüklənməyib');
        }
    },
    ...
    updateTaskCount: function() {
        ...
        // TaskManager-ın öz apiRequest metodundan istifadə et (token avtomatik əlavə olunur)
        if (window.taskManager && typeof window.taskManager.apiRequest === 'function') {
            // limit=1000 ilə sorğu et (backend maksimum 100-ə qədər qəbul edir, amma 100 də 20-dən yaxşıdır)
            window.taskManager.apiRequest('/tasks/detailed?page=1&limit=100&status=pending,in_progress,overdue,waiting,pending_approval,paused', 'GET')
                .then(response => {
                    let tasks = [];
                    if (Array.isArray(response)) {
                        tasks = response;
                    ...
                    // 1. Dalğa naviqasiyasındakı elementi yenilə
                    countElement.textContent = count;

                    // 2. Cədvəl başlığındakı elementi yenilə (əgər varsa)
                    const tableMetaElement = document.getElementById('activeTableTotalCount');
                    if (tableMetaElement) {
                        tableMetaElement.textContent = count;
                    }

                    // 3. Əgər updateTableMeta funksiyası varsa, onu da çağır
                    if (typeof this.updateTableMeta === 'function') {
                        this.updateTableMeta('active', count);
                    }

                    console.log(`📊 Ümumi aktiv task sayı: ${count}`);
                })
                .catch(err => {
                    console.error('❌ Xəta:', err);
                    // Fallback: cədvəldəki taskları say
                    ...
                    const tableMetaElement = document.getElementById('activeTableTotalCount');
                    if (tableMetaElement) {
                        tableMetaElement.textContent = fallbackCount;
                    }

                    if (typeof this.updateTableMeta === 'function') {
                        this.updateTableMeta('active', fallbackCount);
                    }
                });
        } else {
            // Fallback
    ...
    autoRefreshAfterAction: async function(taskId, tableType, actionType = null) {
        try {
            ...
            if (window.taskManager) {
                ...
                const activeEndpoint = '/tasks/detailed?page=1&limit=100&status=pending,in_progress,overdue,pending_approval,waiting,paused';
                const activeResponse = await this.apiRequest(activeEndpoint, 'GET');
                const allTasks = activeResponse?.data || activeResponse || [];
                const activeTasks = allTasks.filter(t => ['pending_approval','pending','in_progress','overdue','waiting','paused'].includes(t.status));
