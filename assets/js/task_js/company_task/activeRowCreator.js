...
window.TaskTimerSystem = window.TaskTimerSystem || {
    ...
    startWorkTimer: function(taskId, startedAt, totalPausedSeconds) {
        ...
        const tick = () => {
            ...
            if (!el) return;
            const elapsed = Math.max(0, Date.now() - t.startTime - t.pausedMs);
            const h = Math.floor(elapsed / 3600000);
            const m = Math.floor((elapsed % 3600000) / 60000);
            const s = Math.floor((elapsed % 60000) / 1000);
            el.textContent = `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;
            if (lbl) lbl.textContent = fmtWorked(elapsed);
        };
...
function buildPendingApprovalHTML(taskId) {
    ...
    if (delayInfo.isDelayed) {
        return `<div data-confirm-timer="${taskId}"><div class="timer-delayed">
            <i class="fas fa-exclamation-triangle"></i>
            <span class="delay-time">Gecikmə: ${delayInfo.delayHours} saat</span>
        </div></div>
        <div class="confirm-actions">
            <button class="btn-approve" data-approve-btn="${taskId}" onclick="handleTaskApprove(${taskId})"><i class="fas fa-check"></i> (Gecikmiş)</button>
            <button class="btn-reject-approval" data-reject-btn="${taskId}" onclick="handleTaskRejectApproval(${taskId})"><i class="fas fa-times"></i></button>
            ${editButton}
        </div>`;
    }

    return `<div data-confirm-timer="${taskId}"><div class="confirm-countdown normal">
        <i class="fas fa-hourglass-half"></i>
        <span class="countdown-time">02:00:00</span>
        <span class="countdown-label">İş vaxtı qaldı</span>
    </div></div>
    <div class="confirm-actions">
        <button class="btn-approve" data-approve-btn="${taskId}" onclick="handleTaskApprove(${taskId})"><i class="fas fa-check"></i> Təsdiq et</button>
        <button class="btn-reject-approval" data-reject-btn="${taskId}" onclick="handleTaskRejectApproval(${taskId})"><i class="fas fa-times"></i> İmtina</button>
        ${editButton}
    </div>`;
}
...
function buildWaitingHTML(taskId, isAssignedToMe) {
    ...
    return `<div class="waiting-section">
        <span class="status-waiting-badge"><i class="fas fa-check-circle"></i>Gözləyir</span>
        <div style="display:flex;gap:8px;align-items:center;margin-top:5px;">
            ${isAssignedToMe ? `<button class="btn-start-work" data-start-btn="${taskId}" onclick="handleStartWork(${taskId})"><i class="fas fa-play"></i></button>` : ''}
            ${editButton}
        </div>
    </div>`;
}
...
function buildWorkingHTML(taskId, isPaused, isAssignedToMe, workedMinutes) {
    ...
    const actionButton = isAssignedToMe ? `
...
const ActiveRowCreator = {
    ...
    createActiveRowHTML: function(task, index, currentPage) {
        ...
        const commentsBtn = `<button class="btn btn-sm btn-info" onclick="TableManager.viewTaskComments(${task.id})" title="Commentlər"><i class="fa-solid fa-comments"></i></button>`;
        ...
        if (task.status === 'pending_approval') {
            statusBadgeHTML = buildPendingApprovalHTML(task.id);
            ...
            if (isOverdue) {
                statusBadgeHTML = `<span class="status-badge status-pending"><i class="fa-solid fa-clock"></i> GECİKMƏ${daysOverdue > 1 ? ` (${daysOverdue}g)` : ''}</span>`;
                ...
                statusButtonHTML = isAssignedToMe
                    ? `<button class="btn btn-sm btn-success" onclick="TableManager.startTaskWithNotification(${task.id},'${(task.task_title||'').replace(/'/g,"\\'")}')"><i class="fa-solid fa-play"></i></button>`
                    : `<button class="btn btn-sm btn-info" onclick="TableManager.takeTaskFromOthers(${task.id})"><i class="fa-solid fa-user-plus"></i></button>`;
            }
        } else if (task.status === 'rejected') {
            statusBadgeHTML = `<span class="status-badge status-rejected"><i class="fa-solid fa-ban"></i> İmtina edildi</span>`;
        } else if (task.status === 'completed') {
            statusBadgeHTML = `<span class="status-badge status-completed"><i class="fa-solid fa-check-circle"></i> Tamamlandı</span>`;
            statusButtonHTML = '';
        } else {
            statusBadgeHTML = `<span class="status-badge" style="background:#e2e8f0;color:#475569;">${task.status}</span>`;
        }
        ...
        let attachments = [];
        if (task.attachments) {
            try { attachments = Array.isArray(task.attachments) ? task.attachments : JSON.parse(task.attachments); } catch(e) {}
        }
        if (attachments.length > 0) {
            const getIcon = f => {
                const mt = f.mime_type || '', fn = f.filename || f.original_filename || '';
                if (f.is_audio_recording || mt.startsWith('audio/')) return '<i class="fas fa-microphone" style="color:#3b82f6;"></i>';
                if (mt.startsWith('image/') || fn.match(/\.(jpg|jpeg|png|gif|webp)$/i)) return '<i class="fas fa-image" style="color:#3b82f6;"></i>';
                if (mt.includes('pdf') || fn.endsWith('.pdf')) return '<i class="fas fa-file-pdf" style="color:#ef4444;"></i>';
                if (mt.includes('excel') || fn.endsWith('.xlsx')) return '<i class="fas fa-file-excel" style="color:#10b981;"></i>';
                if (mt.includes('word') || fn.endsWith('.docx')) return '<i class="fas fa-file-word" style="color:#3b82f6;"></i>';
                return '<i class="fas fa-file" style="color:#64748b;"></i>';
            };
            if (attachments.length === 1) {
                const f = attachments[0], fid = f.file_id || f.uuid || f.id;
                const fn = f.filename || f.original_filename || 'Fayl';
                const sn = fn.length > 10 ? fn.substring(0, 8) + '...' : fn;
                if (fid) fileColumnHTML = `<div style="display:inline-flex;align-items:center;gap:5px;padding:4px 8px;border-radius:4px;background:#eef2ff;border:1px solid #cbd5e1;cursor:pointer;"
                    onclick="TableManager.previewFile('${fid}','${fn}','${f.mime_type||''}',${f.is_audio_recording||false})" title="${fn}">${getIcon(f)}<span style="font-size:12px;color:#1e293b;">${sn}</span></div>`;
            } else {
                fileColumnHTML = `<div style="display:inline-flex;align-items:center;gap:5px;padding:4px 8px;border-radius:4px;background:#f1f5f9;border:1px solid #cbd5e1;cursor:pointer;"
                    onclick="TableManager.showAllFiles(${task.id})"><i class="fas fa-paperclip" style="color:#475569;"></i>
                    <span style="font-size:12px;font-weight:600;color:#334155;">${attachments.length}</span></div>`;
            }
        }
        ...
        return `<tr data-task-id="${task.id}" style="transition:all 0.2s;">
            ...
            <td class="description-col" style="max-width:200px;"><div style="position:relative;">
                <div id="desc-${task.id}" style="display:${description.length>10?'block':'none'};color:#475569;font-size:13px;">${this.truncateText(description,10)}</div>
                <div id="full-desc-${task.id}" style="display:none;color:#475569;font-size:13px;">${this.escapeHtml(description)}</div>
                ${description.length>10?`<button onclick="TableManager.toggleDescription(${task.id})" style="background:none;border:none;color:#3b82f6;font-size:11px;cursor:pointer;padding:2px 0;display:flex;align-items:center;gap:3px;"><i class="fas fa-expand-alt" style="font-size:10px;"></i> Tam bax</button>`:''}
            </div></td>
