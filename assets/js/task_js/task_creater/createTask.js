// createTask.js - Telegram bildiriş funksiyası - TAM VERSİYA

/**
 * Telegram bildirişi göndər - task yaradılan şəxsə (assigned_to)
 */
async function sendTelegramNotification(taskData, isPartner = false) {
    console.log('🔍 sendTelegramNotification çağırıldı, taskData:', JSON.stringify(taskData, null, 2));
    ...
    try {
        ...
        if (!token) {
            console.log('⚠️ Telegram bildirişi üçün token tapılmadı');
            return false;
        }

        // Taskın təyin olunduğu şəxsin ID-si (assigned_to)
        const targetUserId = taskData.assigned_to;
        console.log('👤 Hədəf user ID (assigned_to):', targetUserId);

        if (!targetUserId) {
            console.log('⚠️ Telegram bildirişi üçün hədəf user ID tapılmadı');
            return false;
        }

        // Yaradan məlumatları
        const creatorName = taskData.creator_name || window.taskManager?.userData?.fullName || window.taskManager?.userData?.name || 'Sistem';
        const creatorCompany = taskData.creator_company || window.taskManager?.userData?.companyName || '';
        const creatorPosition = taskData.creator_position || window.taskManager?.userData?.position || '';

        // Bildiriş məlumatlarını hazırla
        const notificationData = {
            task_id: parseInt(taskData.task_id || taskData.id),
            user_id: parseInt(targetUserId),
            task_title: taskData.task_title || 'Yeni Task',
            task_description: taskData.task_description || '',
            priority: taskData.priority || 'medium',
            due_date: taskData.due_date || '',
            action: 'created',
            assigned_by_name: creatorName,
            creator_company: creatorCompany,
            creator_position: creatorPosition,
            company_name: taskData.company_name || window.taskManager?.userData?.companyName || '',
            partner_name: taskData.partner_name || ''
        };

        console.log('📦 Telegram bildiriş data:', notificationData);

        // Bildirişi göndər - SESSİZ REJİMDƏ (xəta göstərmə)
        const response = await makeApiRequest('/telegram/send-notification', 'POST', notificationData, { silent: true });

        console.log('📥 Telegram API cavabı:', response);

        if (response && (response.status === 200 || response.status === 202 || response.success || response.status === 'success')) {
            console.log('✅ Telegram bildirişi uğurla göndərildi');

            // İstəyə bağlı: uğur bildirişi (toast) - SİLƏ BİLƏRSİNİZ
            if (typeof Swal !== 'undefined' && false) { // false etdik, göstərmir
                Swal.fire({
                    title: '✅ Bildiriş göndərildi',
                    text: `Tapşırıq #${taskData.task_id || taskData.id} üçün Telegram bildirişi göndərildi.`,
                    icon: 'success',
                    timer: 2000,
                    toast: true,
                    position: 'top-end',
            ...
            // XƏTA GÖSTƏRMƏ, SADƏCƏ LOG YAZ
            console.log('ℹ️ Telegram bildirişi göndərilmədi (istifadəçi bağlı deyil və ya xəta)');
            return false;
        }

    } catch (error) {
        // XƏTA GÖSTƏRMƏ, SADƏCƏ LOG YAZ
        console.log('ℹ️ Telegram bildirişi göndərilmədi (istifadəçi bağlı deyil)');
        console.debug('Xəta detalları (debug):', error.message);
        return false;
    }
}
...
// createTask.js - QƏTİ AYIRMA VERSİYASI + UUID DƏSTƏYİ
...
async function handleTaskFormSubmit(e) {
    ...
    try {
        ...
        const parentCompanySelect = document.getElementById('parentCompanySelect'); // YENİ: üst şirkətlər üçün select
        ...
        if (partniyorSelect && partniyorSelect.value && partniyorSelect.value !== '') {
            ...
            console.log('✅ PARTNYOR SEÇİLDİ:', { id: selectedPartnerId, name: selectedPartnerName });
        }

        // ✅ ÜST ŞİRKƏT MƏLUMATLARI (YENİ)
        ...
        if (parentCompanySelect && parentCompanySelect.value && parentCompanySelect.value !== '') {
            ...
            console.log('✅ ÜST ŞİRKƏT SEÇİLDİ:', {
                id: selectedParentCompanyId,
                name: selectedParentCompanyName,
                code: selectedParentCompanyCode
            });
        }
        ...
        if (taskId && typeof showNotification === 'function') {
            showNotification(
                `🤝 Partnyor taskı uğurla yaradıldı!\n📌 Partnyor: ${partnerDisplayName}`,
                'success'
            );
        }

        // 🔴 BURADA QƏRAR VERİLİR - HANSI FUNKSİYA ÇAĞIRILACAQ
        if (hasPartner) {
            console.log('🤝 PARTNYOR TASK YARADILIR - createPartnerTaskOnly çağırılır');
            await createPartnerTaskOnly(form, selectedPartnerId, selectedPartnerName);
        }
        else if (hasParentCompany) {
            console.log('🏢 ÜST ŞİRKƏT TASK YARADILIR - createParentCompanyTask çağırılır');
            await createParentCompanyTask(form, {
            ...
            console.log('📝 ADİ TASK YARADILIR - createRegularTaskOnly çağırılır');
            await createRegularTaskOnly(form);
        }
        ...
        if (taskId) {
            ...
            setTimeout(async () => {
                try {
                    ...
                    const result = await sendTelegramNotification(notificationData, false);
...
async function createPartnerTaskOnly(form, selectedPartnerId, selectedPartnerName) {
    ...
    if (taskId) {
        ...
        setTimeout(async () => {
            ...
            const result = await sendTelegramNotification(notificationData, true);
...
/**
 * Audio yüklə və UUID-ni tasks cədvəlinə əlavə et
 */
async function uploadAudioAndUpdateTask(taskId, audioBase64, filename, description, category) {
    try {
        console.log(`🎤 Audio yüklənir: ${filename}`);

        // Category dəyərini düzgün formata çevir
        let validCategory = 'audio';
        ...
        // Tək fayl kimi yüklə (simple-upload ilə)
        const formData = new FormData();
        formData.append('file', audioFile);
        formData.append('category', validCategory);

        console.log(`📤 Simple-upload ilə audio göndərilir: ${filename}`);
        ...
        if (fileData && fileData.file_id) {
            console.log(`✅ Audio yükləndi: ${filename} -> UUID: ${fileData.file_id}`);

            // UUID-ni task-a əlavə et
            await addUuidsToTask(taskId, [fileData.file_id]);
        }
...
/**
 * ✅ YENİ FUNKSİYA: Faylları yüklə və UUID-ləri tasks cədvəlinə əlavə et
 */
async function uploadFilesAndUpdateTask(taskId, category, files) {
    try {
        console.log(`📎 ${files.length} fayl yüklənir və UUID-lər task ${taskId}-yə əlavə edilir...`);

        // ✅ Category dəyərini düzgün formata çevir
        let validCategory = 'other';

        if (category === 'partner' || category === 'active') {
            validCategory = 'company_file';
            console.log(`🔄 Category "${category}" -> "${validCategory}" olaraq dəyişdirildi`);
        } else {
            validCategory = category;
        }

        // ✅ İstifadəçi UUID-sini tap - BÜTÜN MÜMKÜN MƏNBƏLƏRİ YOXLA
        let userUuid = null;
        let userEmail = null;

        // 1. taskManager-dan yoxla
        if (window.taskManager?.userData) {
            console.log('🔍 taskManager.userData:', window.taskManager.userData);

            if (window.taskManager.userData.uuid) {
                userUuid = window.taskManager.userData.uuid;
            } else if (window.taskManager.userData.user?.uuid) {
                userUuid = window.taskManager.userData.user.uuid;
            }

            userEmail = window.taskManager.userData.ceo_email ||
                        window.taskManager.userData.email ||
                        window.taskManager.userData.user?.ceo_email;
        }
        ...
        if (!userUuid) {
            try {
                ...
                if (savedUserData) {
                    ...
                    } else if (parsed.user?.uuid) {
                        userUuid = parsed.user.uuid;
                    }

                    if (!userEmail && parsed.ceo_email) {
                        userEmail = parsed.ceo_email;
                    }
                }
            } catch (e) {
                console.warn('⚠️ localStorage oxunarkən xəta:', e);
            }
        }

        // 3. Token-dan user_id varsa, API-dən UUID al
        if (!userUuid && window.taskManager?.userData?.id) {
            try {
                console.log(`📡 API-dən UUID alınır: /users/${window.taskManager.userData.id}`);
                const response = await window.taskManager.apiRequest(`/users/${window.taskManager.userData.id}`, 'GET');
                const userData = response.data || response;

                if (userData && userData.uuid) {
                    userUuid = userData.uuid;
                    console.log(`✅ API-dən UUID alındı: ${userUuid}`);

                    // localStorage-a yadda saxla
                    try {
                        const savedData = JSON.parse(localStorage.getItem('userData') || '{}');
                        savedData.uuid = userUuid;
                        localStorage.setItem('userData', JSON.stringify(savedData));
                    } catch (e) {}
                }
            } catch (e) {
                console.warn('⚠️ API-dən UUID alınarkən xəta:', e);
            }
        }

        console.log(`👤 İstifadəçi UUID: ${userUuid || 'tapılmadı'}`);
        console.log(`👤 İstifadəçi Email: ${userEmail || 'tapılmadı'}`);

        // 4. Əgər UUID yoxdursa, simple-upload endpoint-indən istifadə et
        if (!userUuid) {
            console.log('⚠️ UUID tapılmadı, /simple-upload endpoint-indən istifadə edilir...');

            // simple-upload endpoint-inə yönləndir
            const uploadedUuids = await uploadFilesViaSimpleUpload(taskId, files, validCategory);

            if (uploadedUuids.length > 0) {
                // UUID-ləri tasks cədvəlinə əlavə et
        ...
        // 1. Faylları yüklə və UUID-ləri al
        const uploadedUuids = [];
        ...
        for (const file of files) {
            try {
                ...
                // ✅ DÜZGÜN owner_type - 'USER' olmalıdır
                formData.append('owner_type', 'USER');

                // ✅ owner_uuid - mütləq göndər
                formData.append('owner_uuid', userUuid);

                // user_type - 'company_user' və ya 'employee'
                formData.append('user_type', window.taskManager.userData?.userType || 'company_user');

                // company_uuid (əgər varsa)
                if (window.taskManager.userData?.companyUuid) {
                    formData.append('company_uuid', window.taskManager.userData.companyUuid);
                }

                // İstəyə bağlı metadata
                ...
                console.log(`📤 Fayl göndərilir: ${file.name}, owner_uuid: ${userUuid}`);

                // Faylı yüklə
                const response = await window.taskManager.apiRequest('/files/upload', 'POST', formData, {
                    'Content-Type': 'multipart/form-data'
                });
                ...
                if (fileData && fileData.uuid) {
                    ...
                    console.log(`✅ Fayl yükləndi: ${file.name} -> UUID: ${fileData.uuid}`);
                } else {
                    console.warn(`⚠️ Fayl yükləndi amma UUID alınmadı: ${file.name}`, fileData);
                }
            } catch (fileError) {
                console.error(`❌ Fayl yüklənmədi: ${file.name}`, fileError);

                // Xəta detallarını göstər
                if (fileError.data?.detail) {
                    console.error('📋 Xəta detalları:', fileError.data.detail);
                }
            }
        }

        // 2. UUID-ləri tasks cədvəlinə əlavə et
        await addUuidsToTask(taskId, uploadedUuids);

    } catch (error) {
        console.error('❌ Fayl yükləmə prosesində xəta:', error);
    }
}
...
async function addUuidsToTask(taskId, uuids) {
    if (uuids.length === 0) {
        console.log('⚠️ Heç bir fayl yüklənmədi');
        return;
    }

    console.log(`📝 ${uuids.length} UUID tasks cədvəlinə əlavə edilir...`);

    // Hamısını bir dəfəyə əlavə et
    try {
        await window.taskManager.apiRequest(`/tasks/${taskId}/add-file-uuids`, 'POST', {
            file_uuids: uuids
        });
        console.log(`✅ ${uuids.length} UUID uğurla əlavə edildi`);
    } catch (batchError) {
        console.error('❌ Çoxlu UUID əlavə edilmədi, tək-tək cəhd edilir...', batchError);

        // Tək-tək əlavə et
        for (const uuid of uuids) {
            try {
                await window.taskManager.apiRequest(`/tasks/${taskId}/add-file-uuid`, 'POST', {
                    file_uuid: uuid
                });
                console.log(`✅ UUID ${uuid} task ${taskId}-yə əlavə edildi`);
            } catch (singleError) {
                console.error(`❌ UUID əlavə edilmədi: ${uuid}`, singleError);
            }
        }
    }

    console.log(`🎉 Ümumi ${uuids.length} UUID uğurla əlavə edildi`);
}

// createTask.js - Cache təmizləmə funksiyası
